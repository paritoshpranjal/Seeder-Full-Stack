...
import com.seeder.userservice.service.implementation.UserServiceImpl;
...
class UserServiceImplTest {
    ...
    @Test
    void testAvaialbleCredit() {
        int userId = 1;

        UserEntity mockUser = new UserEntity();
        mockUser.setId(userId);
        mockUser.setAvailableCredit(10);
        ...
        mockUserDTO.setAvailableCredit(15);

        when(userMapper.entityToDTO(any(UserEntity.class))).thenReturn(mockUserDTO);

        UserDto result = userService.updateAvailableCredit(userId, mockUserDTO);

        assertNotNull(result);
        assertEquals(15, result.getAvailableCredit());
    ...
    @Test
    void testUpdateAvailableCredit_UserNotFound_ThrowsUserNotFound() {
        int userId = 1;
        UserDto mockUserDTO = new UserDto();
        mockUserDTO.setAvailableCredit(5);

        when(userRepository.findById(userId)).thenReturn(Optional.empty());

        assertThrows(UserException.class, () -> userService.updateAvailableCredit(userId, mockUserDTO));
        verify(userRepository, times(1)).findById(userId);
        verify(userRepository, never()).save(any(UserEntity.class));
        verify(userMapper, never()).entityToDTO(any(UserEntity.class));
    }
    ...
    void testUpdatePassword_PasswordChanged_ReturnsUpdatedUserDTO() {
        ...
        var result = userService.updatePassword(userId, userDto);
    ...
    void testUpdatePassword_UserFound_SameAsPreviousPassword_ThrowsUserNotFound() {
        ...
        assertThrows(UserException.class, () -> userService.updatePassword(userId, userDto));
        verify(userRepository, times(1)).findById(userId);
        verify(userRepository, never()).save(any(UserEntity.class));
        verify(userMapper, never()).entityToDTO(any(UserEntity.class));
    }
